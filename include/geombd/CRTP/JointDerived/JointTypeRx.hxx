/**
 *    \file include/geombd/CRTP/JointDerived/JointTypeRx.hxx
...
#ifndef GEOMBD_JOINT_TYPE_REVOLUTE_X_HXX
#define GEOMBD_JOINT_TYPE_REVOLUTE_X_HXX
...
namespace geoCRTP{
  ...
  //! Derived -> Joint Type Revolute On X Axis
  //!------------------------------------------------------------------------------!//
  struct JointTypeRx : public CRTPInterface<JointTypeRx>
  {
  public:
  ...
  template<typename ScalarType, typename Vector3Type, typename Matrix3Type>
  inline void
  JointTypeRx::runFK(const ScalarType & qi,
                     typename Eigen::MatrixBase<Vector3Type> & S,
                     typename Eigen::MatrixBase<Matrix3Type> & R) {
  ...
  template<typename ScalarType, typename Vector3Type, typename Vector6Type, typename Matrix6Type>
  inline void
  JointTypeRx::runTCP_root(const ScalarType & vi,
                           const Eigen::MatrixBase<Vector3Type> & S,
                           Eigen::MatrixBase<Vector6Type> & S_i,
                           Eigen::MatrixBase<Vector6Type> & p_,
                           const Eigen::MatrixBase<Matrix6Type> & M_) {
  ...
  template<typename ScalarType, typename Matrix3Type, typename Matrix6Type,
           typename Vector3Type, typename Vector6Type>
  inline void
  JointTypeRx::runTwCbPb(bool zeroFlag,
                         const ScalarType & vi,
                         const Eigen::MatrixBase<Vector3Type> & S,
                         const Eigen::MatrixBase<Matrix3Type> & R_,
                         const Eigen::MatrixBase<Vector3Type> & P_,
    ...
    Cup.coeffRef(1) =  Up_.coeff(2)*vi;  // -ad(Sx) effect
    Cup.coeffRef(2) = -Up_.coeff(1)*vi;

    Cdown.coeffRef(1) =  Down_.coeff(2)*vi;
    Cdown.coeffRef(2) = -Down_.coeff(1)*vi;
...
//  EIGEN_ALWAYS_INLINE EIGEN_DEVICE_FUNC
  ...
  template<typename ScalarType, typename Vector3Type, typename Vector6Type, typename Matrix6Type>
  inline void
  JointTypeRx::runUuiD(ScalarType & u,
                       ScalarType & iD,
                       const ScalarType tau,
                       const Eigen::MatrixBase<Vector3Type> & S,
                       Eigen::MatrixBase<Vector6Type> & U_r,
  ...
  template<typename ScalarType, typename Vector3Type, typename Vector6Type, typename Matrix6Type>
  inline void
  JointTypeRx::runPreIner(ScalarType u,
                          ScalarType iD,
                          const Eigen::MatrixBase<Vector3Type> & S,
                          const Eigen::MatrixBase<Vector6Type> & U_r,
                          const Eigen::MatrixBase<Vector6Type> & c_r,
  ...
  template<typename Vector3Type, typename Matrix3Type, typename Vector6Type, typename Matrix6Type>
  inline void
  JointTypeRx::runInerProj(bool P_z,
                           const Eigen::MatrixBase<Vector3Type> & S,
                           const Eigen::MatrixBase<Vector3Type> & P_,
                           const Eigen::MatrixBase<Matrix3Type> & R_,
                           const Eigen::MatrixBase<Vector6Type> & P_a_r,
...
//    typename GEOMBD_EIGEN_PLAIN_TYPE(Matrix6Type) Mtmp;
  ...
  template<typename ScalarType, typename Vector3Type, typename Matrix3Type, typename Vector6Type>
  inline void
  JointTypeRx::runAccel(bool zeroFlag,
                        ScalarType u,
                        ScalarType iD,
                        ScalarType* ddq,
                        const Eigen::MatrixBase<Vector3Type> & S,
  ...
  template<typename ScalarType, typename Vector3Type, typename Matrix3Type, typename Vector6Type>
  inline void
  JointTypeRx::runAccelRoot(ScalarType u,
                            ScalarType iD,
                            ScalarType* ddq,
                            const Eigen::MatrixBase<Vector3Type> & S,
                            const Eigen::MatrixBase<Vector3Type> & P_r,
    ...
    ScalarType sq, cq;  sq = R_r.coeff(2,1);  cq = R_r.coeff(1,1);
    Acc_i_r.setZero();
  ...
  template<typename ScalarType, typename Vector3Type, typename Vector6Type, typename Matrix6Type>
  inline void
  JointTypeRx::runUinvD(ScalarType & iD,
                        const Eigen::MatrixBase<Vector3Type> & S,
                        Eigen::MatrixBase<Vector6Type> & U_r,
                        const Eigen::MatrixBase<Matrix6Type> & M_A_r) {
  ...
  template<typename IntType, typename ScalarType, typename Vector3Type, typename D_Vector6Type, typename RowVectorXrType>
  inline void
  JointTypeRx::runInvInertia(IntType n_ID,
                             ScalarType & iD,
                             const Eigen::MatrixBase<Vector3Type> & S,
                             const Eigen::MatrixBase<D_Vector6Type> & Fcrb_r,
                             Eigen::MatrixBase<RowVectorXrType> & iHrow_r) {

    iHrow_r.noalias() = iD*Fcrb_r.row(3).tail(n_ID);
  ...
  template<typename IntType, typename Vector3Type, typename D_Vector6Type, typename MatrixXrType>
  inline void
  JointTypeRx::runHSelector(IntType n,
                            IntType ID,
                            const Eigen::MatrixBase<Vector3Type> & S,
                            Eigen::MatrixBase<D_Vector6Type> & Pc_,
                            const Eigen::MatrixBase<MatrixXrType> & iH_total_) {
    ...
    Pc_.setZero();
    Pc_.block(3,ID,1,n-ID) = iH_total_.block(ID,ID,1,n-ID);
...
#endif // GEOMBD_JOINT_TYPE_REVOLUTE_X_HXX
