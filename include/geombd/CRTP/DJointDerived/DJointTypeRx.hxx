...
namespace geo{
  ...
  template<typename ScalarType, typename Vector3Type, typename Matrix3Type>
  inline void
  D_JointTypeRx::runD_FK(const ScalarType & qi,
                         const Eigen::MatrixBase<Vector3Type> & S,
                         typename Eigen::MatrixBase<Matrix3Type> & R) {
  ...
  template<typename ScalarType, typename Vector3Type, typename Vector6Type, typename Matrix6Type, typename D_Vector6Type>
  inline void
  D_JointTypeRx::runD_TCP_root(const ScalarType & vi,
                               const Eigen::MatrixBase<Vector3Type> & S,
                               Eigen::MatrixBase<Vector6Type> & S_i,
                               Eigen::MatrixBase<Vector6Type> & p_,
                               const Eigen::MatrixBase<Matrix6Type> & M_,
    ...
    p_.coeffRef(1) = -M.coeff(2,3)*vi2;
    p_.coeffRef(2) =  M.coeff(1,3)*vi2;
    p_.coeffRef(4) = -M.coeff(5,3)*vi2;
    p_.coeffRef(5) =  M.coeff(4,3)*vi2;
    ...
    //! adDual
//    D_p_.setZero();
    D_p_.template row(1) = -M.template row(2);  // adDual(Sx) effect
    D_p_.template row(2) =  M.template row(1);
    D_p_.template row(4) = -M.template row(5);
    D_p_.template row(5) =  M.template row(4);

    Vector6Type mu;
    mu.noalias() = M.template col(3)*vi;

    //! adBar
    UR_.noalias() -= Skew(mu.segment(0,3));
    LL_.noalias() -= Skew(mu.segment(0,3));  LR_.noalias() -= Skew(mu.segment(3,3));

    D_dq_p_.noalias() = D_p_.template col(3);
  }
  ...
  template<typename ScalarType, typename Matrix3Type, typename Matrix6Type,
           typename Vector3Type, typename Vector6Type, typename D_Vector6Type>
  inline void
  D_JointTypeRx::runD_TwCbPb(bool zeroFlag,
                             const ScalarType & vi,
                             const Eigen::MatrixBase<Vector3Type> & S,
                             const Eigen::MatrixBase<Matrix3Type> & R_,
                             const Eigen::MatrixBase<Vector3Type> & P_,
    ...
    Cup.coeffRef(1) =  Up_.coeff(2);  // -ad(Sx) effect
    Cup.coeffRef(2) = -Up_.coeff(1);

    Cdown.coeffRef(1) =  Down_.coeff(2);
    Cdown.coeffRef(2) = -Down_.coeff(1);
    ...
    D_q_c_.template row(1) =  D_c_aux.template row(2);  // -ad(Sx)* effect
    D_q_c_.template row(2) = -D_c_aux.template row(1);
    D_q_c_.template row(4) =  D_c_aux.template row(5);
    D_q_c_.template row(5) = -D_c_aux.template row(4);

    D_c_aux.noalias() = D_dq_V_*vi;
    D_c_aux.template rightCols<1>() += S_i;
    D_dq_c_.template row(1) =  D_c_aux.template row(2);  // -ad(Sx)* effect
    D_dq_c_.template row(2) = -D_c_aux.template row(1);
    D_dq_c_.template row(4) =  D_c_aux.template row(5);
    D_dq_c_.template row(5) = -D_c_aux.template row(4);
    //!------------------------------------------------------------------------------!//
  ...
  template<typename ScalarType, typename Vector3Type, typename Matrix3Type, typename Vector6Type,
           typename Matrix6Type, typename RowVectorXType, typename D_Vector6Type, typename D_Matrix6Type>
  inline void
  D_JointTypeRx::runD_InertiaLeaf(ScalarType & u,
                                  ScalarType & iD,
                                  const ScalarType tau,
                                  const Eigen::MatrixBase<Vector3Type> & S,
                                  Eigen::MatrixBase<Vector6Type> & U_,
    ...
    //!------------------------------------------------------------------------------!//
    U_ = M_A_.template col(3);  iD = 1 / U_.coeff(3);  u = tau - P_A_.coeff(3);
    ...
    Mat6ProjRx(P_z, P_, R_, M_a_, Mtmp);
    ...
    Mat6ProjRx(P_z, P_, R_, M_a_S_, Mtmp);
    ...
    //!------------------------------------------------------------------------------!//
    P_A_i << 0, -P_a_.coeff(2), P_a_.coeff(1), 0, -P_a_.coeff(5), P_a_.coeff(4);
  ...
  template<typename ScalarType, typename Vector6iType, typename Vector3Type, typename Matrix3Type, typename Vector6Type,
           typename Matrix6Type, typename VectorXType, typename RowVectorXType, typename D_Vector6Type, typename D_Matrix6Type>
  inline void
  D_JointTypeRx::runD_Inertial(bool rootFlag,
    ...
    iD = 1 / U_.coeffRef(3);
    //!-------------------------------------------------------
    D_invD_.noalias() = -pow(iD,2)*D_U_h_.template row(3);


    //! Solve u and its partial derivative.
    //!------------------------------------------------------------------------------!//
    u = tau - P_A_.coeffRef(3);
    //!-------------------------------------------------------
    D_q_u_ = -D_q_PA_.template row(3);  D_dq_u_ = -D_dq_PA_.template row(3);
    ...
    if (rootFlag) {
        ...
        D_Mat6ProjRx(P_z, nS, P_, R_, D_MtmpII, D_MtmpI);
  ...
  D_JointTypeRx::runD_AccelRoot(ScalarType u,
    ...
    //! D_Acc_a = -ad(Sz)*Ad(G)*g = [ 0, 9.81*cq, -9.81*sq, 0, 0, 0 ]^T
  ...
  template<typename ScalarType, typename IndexType, typename Vector3Type, typename Matrix3Type,
           typename Vector6Type, typename D_Vector6Type, typename RowVectorXType, typename MatrixXType>
  inline void
  D_JointTypeRx::runD_Accel(IndexType ID,
                            bool zeroFlag,
                            ScalarType u,
                            ScalarType iD,
                            ScalarType* ddq,
    ...
    AdAj << 0, Aa_.coeff(2), -Aa_.coeff(1), 0, Aa_.coeff(5), -Aa_.coeff(4);
    //!-------------------------------------------------------
    ...
    if (isLeaf) {
        ...
        //!------------------------------------------------------------------------------!//
        D_q_A_ = D_q_Aa_;
        D_q_A_.template row(3) += D_q_ddq;  //! TODO: D_q_A_ can be tmp used as D_q_Aa_; saving an assignation

        D_dq_A_ = D_dq_Aa_;
        D_dq_A_.template row(3) += D_dq_ddq;
      }
